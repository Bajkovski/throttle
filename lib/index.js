...
function verify(options) {
  ...
  if (options.localhost) {
    ...
    (options.packetLoss && !Number.isInteger(options.packetLoss)) ||
    (options.jitter && !Number.isInteger(options.jitter))
  ) {
    throw new Error('Input values needs to be integers');
  } else if (!options.up && !options.down && !options.rtt && !options.stop && !options.packetLoss && !options.jitter) {
